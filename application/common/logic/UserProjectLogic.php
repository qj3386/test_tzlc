...
class UserProjectLogic extends BaseLogic
{
    ...
    public function add($data = array(), $type = 0)
    {
        ...
        $project = model('Project')->getOne(['id' => $data['project_id'], 'delete_time' => 0]);
        if (!$project) {
            return ReturnData::create(ReturnData::FAIL, null, '项目不存在');
        }
        ...
        if ($project['dividend_mode'] == 0) {
            ...
            $user_project_income_data['money'] = round((($project['daily_interest'] + $hike) * $project['term'] * $data['money']) / 100, 2); //利息
            ...
            $user_project_income_data['title'] = $project['title'];
            $user_project_income_data['is_last'] = 1;
            $user_project_income_data['money'] = (round(pow((1 + ($project['daily_interest'] + $hike) * 0.01), $project['term']) * $data['money'], 2) - $data['money']); //复利总金额
            $user_project_income_data['add_time'] = $expire_time; //应支付利息时间
            $res = model('UserProjectIncome')->add($user_project_income_data, 0);
            if (!$res) {
                // 回滚事务
            ...
            for ($x = 1; $x <= $total_period_decrement; $x++) {
                ...
                $temp['is_last'] = 0;
                $temp['money'] = round((($project['daily_interest'] + $hike) * $project['dividend_mode'] * $data['money']) / 100, 2); //利息
            ...
            $temp['title'] = $project['title'];
            $temp['is_last'] = 1;
            $temp['money'] = round((($project['daily_interest'] + $hike) * ($project['term'] - $total_period_decrement * $project['dividend_mode']) * $data['money']) / 100, 2); //利息
        ...
        $user_money_data['desc'] = '投资项目-' . $project['title'];
        ...
        //项目销量+1，库存-1
        model('Project')->edit(['sale' => ($project['sale'] + 1), 'stock' => ($project['stock'] - 1)], ['id' => $data['project_id']]);
        //增加用户累计消费金额
        model('User')->setIncrement(array('id' => $data['user_id']), 'consumption_money', $data['money']);
        //修改用户等级
        logic('User')->user_rank_calculation($data['user_id']);

        //分销计算
        logic('User')->dividend_calculation($data['user_id'], $data['money'], ['distribution_yiji'=>$project['distribution_yiji'], 'distribution_erji'=>$project['distribution_erji']]);
    ...
    public function revenue_calculation($user_id)
    {
        ...
        foreach ($user_project_income as $k => $v) {
            ...
            $user_money_data['desc'] = '投资收益-' . $v['title'];
            ...
            if ($v['is_last'] == 1) {
                ...
                $user_money_data['desc'] = '返还项目本金-' . $v['title'];
                $user_money_data['add_time'] = $v['add_time'];
                $res = logic('UserMoney')->add($user_money_data);
                if ($res['code'] != ReturnData::SUCCESS) {
                    Db::rollback();
