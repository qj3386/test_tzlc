...
class VerifyCode extends Base
{
    ...
    public function getVerifyCodeBySmsbao($mobile, $type, $text = '')
    {
        ...
        return ReturnData::create(ReturnData::SUCCESS, array('code' => $data['code']), '验证码已发送');
    }
	
	public function send_smsbao($code, $mobile, $text = '')
	{
		if ($text == '') {
			$text = '【' . sysconfig('CMS_WEBNAME') . '】您的验证码是' . $code . '，有效期30分钟，若非本人操作请忽略此消息。';
		}

		//短信发送验证码
        $smsbao_config = explode(",", str_replace("，",",", sysconfig('CMS_SMSBAO')));
        $smsbao = new \app\common\service\Smsbao($smsbao_config[0], $smsbao_config[1]);
	...
	public function send_smschinese($code, $mobile, $text = '')
	{
		if ($text == '') {
			$text = '您的验证码是' . $code . '，有效期30分钟，若非本人操作请忽略此消息。';
		}
		...
		if (strlen($mobile) == 11) {
			//添加短信发送记录
			$sms_log_data['mobile'] = $mobile;
			$sms_log_data['content'] = $text;
			$sms_log_data['status'] = 1;
			$sms_log_data['add_time'] = time();
			logic('SmsLog')->add($sms_log_data);
		}
